...
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://localhost/jjvandalHW5"
## Provided:
...
def update(item):
    ...
    todo_item = TodoItem.query.filter_by(id = item).first()
    if form.validate_on_submit():
        upd_priority = form.priority.data
        todo_item.priority = upd_priority
        db.session.add(todo_item)
        db.session.commit()
        flash('Updated priority of: \'' + todo_item.description + '\'')
        return redirect(url_for('all_lists'))
...
def delete(lst): #Form??
    del_lst = TodoList.query.filter_by(id = lst).first()
    db.session.delete(del_lst)
    db.session.commit()
    flash('Successfully deleted: \'' + del_lst.title + '\'')
    return redirect(url_for('all_lists'))
    # Replace with code
